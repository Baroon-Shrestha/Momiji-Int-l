  ...
  return (
    <div className="w-full px-2 sm:px-4 lg:px-6">
        ...
        {slides.map((slide, index) => (
          <div
            ...
            className={`relative rounded-2xl overflow-hidden cursor-pointer shadow-lg transition-all duration-300 transform hover:scale-[1.02] ${
              active === index ? "h-auto min-h-[140px]" : "h-24"
            }`}
          >
            <img
              src={slide.image}
      ...
      <div className="hidden lg:block">
        ...
        <div className="flex h-[450px] w-full container mx-auto overflow-hidden rounded-2xl shadow-2xl relative">
          {slides.map((slide, index) => (
            <div
              key={index}
              onClick={() => handleSlideClick(index)}
              className={`relative transition-all duration-700 ease-out cursor-pointer overflow-hidden group
                ${
                  active === index
                    ? "flex-[4] bg-black"
